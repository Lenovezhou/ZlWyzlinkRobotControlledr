...
namespace HoloToolkit.Unity.InputModule
{
    ...
    public class HandGuidance : Singleton<HandGuidance>
    {
        ...
        private void InteractionManager_SourceUpdated(UnityEngine.XR.WSA.Input.InteractionSourceUpdatedEventArgs hand)
        {
            ...
            if (!currentlyTrackedHand.HasValue)
            {
                currentlyTrackedHand = hand.state.source.id;
            }
            else if (currentlyTrackedHand.Value != hand.state.source.id)
            {
            ...
            if (hand.state.properties.sourceLossRisk > HandGuidanceThreshold)
            {
                ShowHandGuidanceIndicator(hand.state);
            }
            else
            {
                HideHandGuidanceIndicator(hand.state);
            }
        }

        private void InteractionManager_SourceReleased(UnityEngine.XR.WSA.Input.InteractionSourceReleasedEventArgs hand)
        {
            // Stop displaying the guidance indicator when the user releases their finger from the pressed state.
            RemoveTrackedHand(hand.state);
        }

        private void InteractionManager_SourceLost(UnityEngine.XR.WSA.Input.InteractionSourceLostEventArgs hand)
        {
