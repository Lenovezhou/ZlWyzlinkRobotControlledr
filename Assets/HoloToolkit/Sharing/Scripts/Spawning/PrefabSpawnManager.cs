...
namespace HoloToolkit.Sharing.Spawning
{
    ...
    public class PrefabSpawnManager : SpawnManager<SyncSpawnedObject>
    {
        ...
        private List<string> alreadyhave = new List<string>();
        /// <summary>
        ...
        protected override void InstantiateFromNetwork(SyncSpawnedObject spawnedObject)
        {
           ...
           GameObject go = CreatePrefabInstance(spawnedObject, prefab, parent, spawnedObject.Name.Value);
            if (VuforiaManager.Instance)
                VuforiaManager.Instance.RegisterSawpobj(go.transform);
        }
        ...
        {
            if (alreadyhave.Contains(objectName))
            {
                return null;
            }
