...
namespace SpectatorView
{
    ...
    public class PlayerController : NetworkBehaviour
    {
        ...
        /// <summary>
        /// client runprogramline 同步机制
        /// </summary>
        public RobotAggregator robotAggregator;
        ...
        [SyncVar(hook = "OnChangesyncProgramLine")]
        private string syncProgramLine = "";
        ...
        [SyncVar]
        private bool isSystemDancing = false;

        [SyncVar]
        private bool isSystemPause = false;
...
#region Spatial Mapping
        ...
        private void OnDestroy()
        {
            ...
            if (HostIP == PlayerIp || HostIP == PlayerName)
            {
                ...
                if (SpectatorViewManager.Instance)
...
#region Spatial Mapping
        ...
        private void CmdSendSharedTransform(GameObject target, Vector3 pos, Quaternion rot)
        {
            ...
            GizmoController gc = target.GetComponent<GizmoController>();
            if (gc)
            {
                gc.CmdSendSyncTransform(pos, rot);
            }
        }
        ...
        [Command]
        public void CmdSetupRobotScene(Vector3 pos,Vector3 _forward)
        {
            SyncedRobotController syncrobot = GameObject.FindObjectOfType<SyncedRobotController>();
            if (null == syncrobot)
            {
                GameObject robot = Instantiate(SyncRobot);
                robot.transform.position = pos;
                robot.transform.forward = _forward;
        ...
        [Command]
        public void CmdSpawnGizmos(Vector3 _forward, Vector3 startPosition, Vector3 endpos, GizmoController.GizmoPostAction gizmoPostAction, bool isdancing)
        {
            GameObject gizmo = Instantiate(Gizmo);
            gizmo.transform.forward = _forward;
            gizmo.transform.position = startPosition;
            GizmoController gc = gizmo.GetComponent<GizmoController>();

            Run.After(1, () => gc.CmdSetPostAction(gizmoPostAction, isdancing));
            NetworkServer.Spawn(gizmo);

            RpcLerpGizmos(gizmo, startPosition, endpos);
        }


        [ClientRpc]
        public void RpcLerpGizmos(GameObject gizmo, Vector3 startpos, Vector3 endpos)
        {
            Run.Lerp(0.5f, (t) => { gizmo.transform.position = Vector3.Lerp(startpos,endpos,t); });
        }

        void CheckRobotAggetator()
        {
            robotAggregator = GameObject.FindObjectOfType<RobotAggregator>();
        }

        public void OnChangesyncProgramLine(string newProgramLine)
        {
            if (!robotAggregator)
            {
                CheckRobotAggetator();
            }
            syncProgramLine = newProgramLine;
            this.robotAggregator.RunProgramLine(newProgramLine);
        }

        [Command]
        public void CmdSetProgramLine(string newprogram)
        {
            this.syncProgramLine = newprogram;
        }

        [Command]
        public void CmdDestroySync(GameObject go)
        {
            Destroy(go);
        }

        [Command]
        public void CmdSyncSetPostAction(GameObject go,GizmoController.GizmoPostAction postAction,bool isdancing)
        {
            GizmoController gc = go.GetComponent<GizmoController>();
            if (gc)
                gc.CmdSetPostAction(postAction,isdancing);
        }

        [Command]
        public void CmdSyncPlaySound(bool issystemdancing)
        {
            //this.musicpath = path;
            RpcPlaySound(issystemdancing);
        }
        [ClientRpc]
        public void RpcPlaySound(bool issystemdancing)
        {
            isSystemDancing = issystemdancing;
            string path = "";
            if (isSystemDancing)
            {
                bool issolomode = RobotsMain.Instance.issolomode;
                path = issolomode ? "SoloDancing" : "Dancing";
            }
            else {
                path = "";
            }
            Sound.Instance.PlayerBG(path);
        }
        public void SyncPlaySound(bool issystemdancing)
        {
            CmdSyncPlaySound(issystemdancing);
        }
