...
namespace SpectatorView
{
    public class ScrollingSessionListUIController : Singleton<ScrollingSessionListUIController>
    {
        ...
        private bool hasshowloadingdialog = false;
        ...
        {
            bool isactive = networkDiscovery.running && !networkDiscovery.isServer;
            SetChildren(isactive);
            if (!isactive && !RobotsMain.Instance.issolomode && !hasshowloadingdialog)
            {
                hasshowloadingdialog = true;
        ...
        public void SetSelectedSessionAndConnectFirst()
        {
            NetworkDiscoveryWithAnchors.SessionInfo info = null;
            foreach (KeyValuePair<string,NetworkDiscoveryWithAnchors.SessionInfo> item in sessionList)
            {
                info = item.Value;
            }
            if (info != null)
            {
                ...
                Debug.Log("sessionList中至少要有一个session");
            }
        }
